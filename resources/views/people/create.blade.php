    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Novo Cadastro de Pessoa / Empresa') }}
            </h2>
            ...
            <div class="bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                <form method="POST" action="{{ route('people.store') }}" class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        @error('name')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div x-data="{
                            ...
                            <input type="text" id="document" name="document" x-model="doc" @input="doc = mask($event.target.value)" placeholder="000.000.000-00 ou 00.000.000/0001-00" required maxlength="18" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            @error('document')
                                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            @error('email')
                                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="phone" name="phone" value="{{ old('phone') }}" placeholder="(00) 00000-0000" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                            @error('phone')
                                <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
